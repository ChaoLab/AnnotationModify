...
my @Sample = qw/CymD.C.HRP CymD.C.MRP CymD.C.LRP CymD.C.HB CymD.C.LB CymS.C.HRP.1 CymS.C.HRP.2 CymS.C.LRP.1 CymS.C.LRP.2 CymS.C.B CymS.C.NBB.1 CymS.C.NBB.2 GyBn.C.APB GyBn.C.NBP
/;

open INN,"ls *PAH_metabolism.result.txt | ";
while (<INN>){
	chomp;
	my $input = $_;
